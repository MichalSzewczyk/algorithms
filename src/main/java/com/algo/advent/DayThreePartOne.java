...
public class DayThreePartOne {
    public static void main(String[] args) {
        ...
        for (int idx = 0; idx < lineChars.length; idx++) {
            lineChars[idx] = lines[idx].toCharArray();
        }
    ...
    private static boolean isMagic(int[] number, char[] lineChar) {
        ...
        int firstIdx = Integer.max(0, lastDigitIndex - digits);
        int lastIdx = Integer.min(lastDigitIndex + 1, lineLength - 1);
    ...
    private static List<int[]> extractNumbersWithIndexes(String[] lines, char[][] lineChars) {
        ...
        for (int lineIdx = 0; lineIdx < lines.length; lineIdx++) {
            int currentNumber = 0;
            for (int charIdx = 0; charIdx < lineChars[lineIdx].length; charIdx++) {
                char character = lineChars[lineIdx][charIdx];
                if (Character.isDigit(character)) {
                    currentNumber *= 10;
                    currentNumber += character - '0';
                } else if (currentNumber != 0) {
                    numbers.add(new int[]{lineIdx, charIdx - 1, currentNumber});
                    currentNumber = 0;
                }
            }
            if (currentNumber != 0) {
                numbers.add(new int[]{lineIdx, lineChars[lineIdx].length - 1, currentNumber});
            }
        }
        return numbers;
    }
